import{_ as n,c as s}from"./app.5561d6b0.js";const a={},p=s(`<h1 id="transform-dsa-and-storage-data" tabindex="-1"><a class="header-anchor" href="#transform-dsa-and-storage-data" aria-hidden="true">#</a> Transform DSA and storage data</h1><p>The CICS Storage dashboards requires a combination of DSA and storage data. In order to achieve this we need to add and run a transform.</p><h2 id="manage-transformed-data" tabindex="-1"><a class="header-anchor" href="#manage-transformed-data" aria-hidden="true">#</a> Manage transformed data</h2><p>The transformed data should be managed similar to our CICS Performance Analyzer data. In order to achieve this, we need to perform the following steps.</p><ol><li><p>Create an index lifecycle management policy by cloning the <code>cicspa-ilm-policy</code>. In kibana, select Management \u25B6 Stack Management \u25B6 Data: Index Lifecycle Policies \u25B6 <code>cicspa-ilm-policy</code> \u25B6 Save as new policy \u25B6 enter <code>transform-cicspa-dsa-storage-ilm-policy</code> as policy name \u25B6 Save as new policy.</p></li><li><p>Create an index template for the transformed data.</p></li></ol><p>Here is a starter index template for the transformed data that use an alias instead of a data stream for rollover:</p><div class="language-json ext-json line-numbers-mode"><pre class="language-json"><code>PUT _index_template/transform-cicspa-dsa-storage
